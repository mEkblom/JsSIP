...
RequestSender.prototype = {
  ...
  */
  receiveResponse: function(response) {
    ...
    */
    if ((status_code === 401 || status_code === 407) && (this.ua.configuration.password !== null || this.ua.configuration.authentication_hash !== null)) {
...
RTCMediaHandler.prototype = {
  ...
  */
  getUserMedia: function(onSuccess, onFailure, constraints) {
    ...
    this.session.emit('mediaRequested',this.session,constraints);

    JsSIP.WebRTC.getUserMedia(constraints,
      function(stream) {
        self.logger.log('got local media stream');
        self.localMedia = stream;
        self.session.emit('mediaGranted',self.session,constraints);
        onSuccess(stream);
      },
      function(e) {
        self.logger.error('unable to get user media');
        self.logger.error(e);
        self.session.emit('mediaDenied',self.session,e);
        onFailure();
      }
    );
...
UA.prototype.loadConfig = function(configuration) {
  ...
  this.contact = {
    pub_gruu: null,
    temp_gruu: null,
    uri: settings.authorization_user ?
      new JsSIP.URI('sip', settings.uri.user, settings.hostport_params, null, {}) :
      new JsSIP.URI('sip', JsSIP.Utils.createRandomToken(8), settings.via_host, null, {transport: 'ws'}),
...
// Horrible Plugin requirement, these must be declared globally
function plugin0() {
  return document.getElementById('_Tem_plugin0');
}
plugin = plugin0;

// Namespace for basic WebRTC functionality
(function(JsSIP) {
var WebRTC;

// Check for plugin supporting WebRTC
if(document.body && 'WebSocket' in window && 'ActiveXObject' in window) {
  try {
    var plugin = new ActiveXObject('Tem.TemWebRTCPlugin');
    if(plugin) {
      WebRTC.PlugId = Math.random().toString(36).slice(2);

      var temp = document.createElement('div');
      temp.innerHTML = '<object id="_Tem_plugin0" type="application/x-temwebrtcplugin" width="1" height="1">' + 
        '<param name="pluginId" value="_Tem_plugin0" /> ' + 
        '<param name="windowless" value="false" /> ' + 
        '<param name="pageId" value="' + WebRTC.PlugId + '" /> ' + 
      ...
      WebRTC.Plugin = document.body.appendChild(temp.firstChild);
    }
  } catch(e) {}
}
...
else if (WebRTC.Plugin && WebRTC.Plugin.getUserMedia) {
  WebRTC.getUserMedia = function(constraints, successCallback, failureCallback) {
    constraints.audio = constraints.audio || false;
    constraints.video = constraints.video || false;
    WebRTC.Plugin.getUserMedia(constraints, successCallback, failureCallback);
  };
}
...
else if(WebRTC.Plugin && WebRTC.Plugin.PeerConnection) {
  WebRTC.RTCPeerConnection = function(servers, constraints) {
    var iceServers = null;
    if (servers) {
      ...
      for (var i = 0; i < iceServers.length; i++) {
        ...
        iceServers[i].hasCredentials = !!(iceServers[i].username && iceServers[i].credential);
      }
    }
    ...
    return WebRTC.Plugin.PeerConnection(WebRTC.PlugId, iceServers, mandatory, optional);
  };
}
...
else if (WebRTC.Plugin && WebRTC.Plugin.ConstructSessionDescription) {
  WebRTC.RTCSessionDescription = function(info) {
    return WebRTC.Plugin.ConstructSessionDescription(info.type, info.sdp);
  };
} 
...
// If using plugin also add the RTCIceCandidate
if(WebRTC.Plugin && WebRTC.Plugin.ConstructIceCandidate) {
  window.RTCIceCandidate = function(candidate) {
    if (!candidate.sdpMid) {
      candidate.sdpMid = '';
    }
    return WebRTC.Plugin.ConstructIceCandidate(candidate.sdpMid, candidate.sdpMLineIndex, candidate.candidate);
  };
}
